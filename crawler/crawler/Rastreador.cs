        ...
        public void Execute()
        {
            ...
            using (WebClient client = new WebClient())
            {
                ...
                foreach (var node in nodes)
                {
                    ...
                    var arr = regex.Matches(nodeImg.InnerHtml).OfType<Match>().Select(m => m.Groups["href"].Value).ToArray();

                    var urlAfiliado = arr[0] + "&afiliado=1";
                    var nome = arr[2];

                    var nodeBoxPreco = node.SelectSingleNode("//a[@class='ui-search-result__content ui-search-link']");
                    var preco = nodeBoxPreco.SelectSingleNode("//span[@class='price-tag-fraction']").InnerText;
                    ...
                    Console.WriteLine($"Seu produto é {nome} e o preço dele é {preco}");
